...
export class CultureSelectionModal {
  constructor(private container: HTMLElement, private onStart: () => void) {}
  ...
  public show(): void {
      ...
      <div class="settings-modal-overlay"><div class="settings-window culture-selection-window">
        ...
        <div class="settings-body"><p class="culture-intro">${t('people.intro')}</p>
          <div class="culture-grid">
            <article class="culture-card culture-card-active"><span class="culture-status">01 · ${t('people.badge')}</span><h3>${t('people.mexica')}</h3>
              <p>${t('people.mexicaDesc')}</p><p class="culture-name-note">${t('people.note')}</p>
              <button class="btn-menu-primary" id="culture-start-mexica">${t('people.start')}</button></article>
            <article class="culture-card culture-card-planned"><span class="culture-status">${t('people.soon')}</span><h3>${t('people.inca')}</h3></article>
    ...
    this.container.querySelector('#culture-start-mexica')?.addEventListener('click', () => {
      GameState.getInstance().selectCulture('mexica'); AudioManager.getInstance().playClick(); this.close(); this.onStart();
    });
    this.container.querySelector('#culture-close')?.addEventListener('click', () => this.close());
    this.container.querySelector('#culture-back')?.addEventListener('click', () => this.close());
  }
