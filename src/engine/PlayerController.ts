...
export const CULTURE_SPAWN_POINTS: Record<string, { x: number; y: number; z: number; rotationY: number }> = {
  mexica: { x: 0, y: 0.7, z: 6, rotationY: Math.PI },
  inca: { x: 0, y: 0.7, z: 10, rotationY: Math.PI },
  maya: { x: 0, y: 0.7, z: 10, rotationY: Math.PI },
  amazonia: { x: 0, y: 0.7, z: 10, rotationY: Math.PI },
  indigenas_brasil: { x: 0, y: 0.7, z: 10, rotationY: Math.PI },
  quechua: { x: 0, y: 0.7, z: 10, rotationY: Math.PI },
  aymara: { x: 0, y: 0.7, z: 10, rotationY: Math.PI },
  zapotec: { x: 0, y: 0.7, z: 10, rotationY: Math.PI },
  raramuri: { x: 0, y: 0.7, z: 10, rotationY: Math.PI },
  bribri: { x: 0, y: 0.7, z: 10, rotationY: Math.PI },
  wayuu: { x: 0, y: 0.7, z: 10, rotationY: Math.PI },
  warao: { x: 0, y: 0.7, z: 10, rotationY: Math.PI },
  yanomami: { x: 0, y: 0.7, z: 10, rotationY: Math.PI },
  guarani: { x: 0, y: 0.7, z: 10, rotationY: Math.PI },
  tupinamba: { x: 0, y: 0.7, z: 10, rotationY: Math.PI },
  xukuru: { x: 0, y: 0.7, z: 10, rotationY: Math.PI },
  default: { x: 0, y: 0.7, z: 10, rotationY: Math.PI }
};
...
export class PlayerController {
  ...
  constructor(cultureId: CultureId = 'inca') {
    ...
    // Posiciona o jogador com ponto de surgimento seguro da cultura selecionada
  ...
  public spawnForCulture(cultureId: CultureId): void {
    const spawn = CULTURE_SPAWN_POINTS[cultureId] || CULTURE_SPAWN_POINTS.default;
    this.group.position.set(spawn.x, spawn.y, spawn.z);
    this.group.rotation.y = spawn.rotationY;
  ...
  public jump(): void {
    if (this.group.position.y <= this.groundY + 0.02) {
  ...
  public resolveOverlap(): void {
    ...
    for (const box of this.colliders) {
      ...
      if (box.intersectsBox(pBox)) {
        const leftPush = box.min.x - (this.group.position.x + radius);
        const rightPush = box.max.x - (this.group.position.x - radius);
        const topPush = box.min.z - (this.group.position.z + radius);
        const bottomPush = box.max.z - (this.group.position.z - radius);

        const pushX = Math.abs(leftPush) < Math.abs(rightPush) ? leftPush : rightPush;
        const pushZ = Math.abs(topPush) < Math.abs(bottomPush) ? topPush : bottomPush;
  ...
  private canOccupy(x: number, z: number): boolean {
    ...
    for (const box of this.colliders) {
      if (box.intersectsBox(targetBox)) {
        // Se a posição atual já estiver em interseção com este obstáculo,
        // permite o movimento se ele afastar o jogador do centro do colisor (escape)
        const curBox = new THREE.Box3(
          new THREE.Vector3(this.group.position.x - radius, 0, this.group.position.z - radius),
          new THREE.Vector3(this.group.position.x + radius, 1.5, this.group.position.z + radius)
        );
        if (box.intersectsBox(curBox)) {
          const boxCenterX = (box.min.x + box.max.x) / 2;
          const boxCenterZ = (box.min.z + box.max.z) / 2;
          const curDistSq = (this.group.position.x - boxCenterX) ** 2 + (this.group.position.z - boxCenterZ) ** 2;
          const nextDistSq = (x - boxCenterX) ** 2 + (z - boxCenterZ) ** 2;
          if (nextDistSq > curDistSq) {
            continue; // Movendo para fora do obstáculo!
          }
        }
        return false;
      }
  ...
  public update(delta: number): void {
    ...
    if (isMoving) {
      ...
      // Limites do espaço jogável (-24 a +24 em X, -24 a +24 em Z)
    ...
    // Resolução contínua de sobreposição para impedir travamento em paredes
    this.resolveOverlap();
